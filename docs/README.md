# Documentation Index

This index links the core Vidra Core documentation by area.

## Start Here

- [Architecture Overview](architecture.md)
- [Project Audit & Improvement Plan](project-management/AUDIT_IMPROVEMENT_PLAN.md)
- [Project Management Overview](project-management/README.md)

## Development

- [Development Guide](development/README.md)
- [Testing Strategy](development/TESTING_STRATEGY.md)
- [Test Baseline Report](development/TEST_BASELINE_REPORT.md)
- [CI/CD Configuration](development/CI_CD_CONFIGURATION.md)
- [Validation Requirements](development/VALIDATION_REQUIRED.md)
- [Validation Quick Reference](development/VALIDATION_QUICK_REFERENCE.md)

## API and Compatibility

- [API Reference Index](../api/README.md) — 39 OpenAPI specs covering full PeerTube v8.1.0 API
- [API Examples](API_EXAMPLES.md)
- [API Contract Policy](API_CONTRACT_POLICY.md)
- [Notifications API Notes](NOTIFICATIONS_API.md)
- [Email Verification API](EMAIL_VERIFICATION_API.md)
- [OAuth2 Guide](OAUTH2.md)
- [PeerTube Compatibility](PEERTUBE_COMPAT.md)
- [PeerTube Migration Guide](PEERTUBE_MIGRATION.md)
- [PeerTube Parity Gap Report](reports/peertube-parity-gap-report.md) — 100% parity validation report

## Deployment and Operations

- [Deployment Overview](deployment/README.md)
- [Production Deployment](deployment/PRODUCTION.md)
- [Operations Runbook](operations/RUNBOOK.md)
- [Monitoring](operations/MONITORING.md)

## Security

- [Security Policy](security/SECURITY.md)
- [Security Advisory](security/SECURITY_ADVISORY.md)
- [E2EE Security](security/SECURITY_E2EE.md)
- [Credential Rotation Guide](security/CREDENTIAL_ROTATION_GUIDE.md)
- [Messaging Security Review](security/MESSAGING_SECURITY_REVIEW.md)

## Federation

- [Federation Overview](federation/README.md)
- [ATProto Setup](federation/ATPROTO_SETUP.md)
- [ActivityPub Coverage](federation/ACTIVITYPUB_TEST_COVERAGE.md)

## Reports

- [PeerTube Parity Gap Report](reports/peertube-parity-gap-report.md) — 100% parity achieved (44 categories, 88 test packages)
- [Postman E2E Test Scenarios](reports/e2e/POSTMAN_E2E_TEST_SCENARIOS.md)
- [Security Audit Summary](reports/security/SECURITY_AUDIT_SUMMARY.md)

## Planning and Delivery

- [Sprint Documentation](sprints/README.md)
- [Quality Programme Working Docs](sprints/quality-programme/README.md)

## Governance

- [Contributing Guide](../CONTRIBUTING.md)
- [Code of Conduct](../CODE_OF_CONDUCT.md)
