...
| `plugin` (`categories`) | 1 row | the instance's category taxonomy | **carried** (see below) |
| `actorImage` | 12,883 rows, ~1,362 files | account + channel avatars and banners | **carried** (see below) |
| `storyboard` | 1,564 | regenerated by Vidra, so no real loss | n/a |
| `videoSource` | 623 | original-file provenance records | still unread |

Every one now carried runs as a **pass of its own**, after its parents, keyed
by their own ledger rows — not as extra work inside the video or account import.
That is what lets a re-run **backfill them onto entities an earlier release
already imported**. An instance whose catalogue is already in Vidra gets them by
re-running the importer; nothing has to be re-imported from scratch.
...
Vidra's built-in list uses PeerTube's ids on purpose. That only works while the
source runs the stock 1–18 list. An instance running
**`peertube-plugin-categories`** does not: it deletes the stock entries and
defines its own at higher ids (the instance measured deletes all eighteen and
adds fourteen at 51–65), so every imported video pointed at an id that validated
against nothing and rendered as no category at all.

The taxonomy is now carried too, into the `instance_custom_categories` setting,
which **replaces** the built-in list when set. It is read from the source's
`plugin` row — settings key `json-categories-as-text`, whose value is a JSON
string that has to be parsed as JSON a second time — and both halves are honoured:
`add` defines the instance's categories, `delete` withdraws stock ids, and what
Vidra ends up offering is what the source offers.

Two rules govern the re-run, because this is one setting an operator also edits
by hand:

- **A source without the plugin gets no override.** Absent, disabled,
  uninstalled, or carrying no taxonomy all read the same way: Vidra's built-in
  list stands and nothing is written. Same when the source's plugin describes
  exactly the built-in list — an override restating it would freeze a shipped
  list against every future change to it.
- **An operator's taxonomy is never clobbered.** The ledger records the exact
  value the import applied (migration 0113). A stored value that still equals it
  is the import's own and is updated when the source moves; anything else — an
  edit, a hand-configured taxonomy that predates the import, a key the operator
  cleared — is left exactly as it is, and the run reports it under `conflicts`
  so the divergence is visible rather than silent. Nothing is ever *removed*
  either: a source that drops its plugin mid-migration leaves the last carried
  taxonomy in place, because the videos already imported still carry its ids.

Run through the **admin API / worker**, the write takes effect immediately (the
server reloads its settings overlay). Run from the **CLI** against a database a
server is already serving, the row is written but that server keeps its cached
overlay until it restarts or an admin saves any instance setting — so restart it,
or expect the new categories to appear at the next boot.

### Instance name, description and terms are NOT in the database

Checked on a live source: they are not there to carry. `application.configPart`
holds only object-storage configuration, and the instance's identity —
`instance.name`, `instance.description`, `instance.terms`, the admin contact —
lives in **`config/local-production.json` on the source host**, which
`--source-dsn` cannot reach by design (the migration only needs the database, and
usually reaches it through an SSH tunnel).
