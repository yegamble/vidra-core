# DigitalOcean test replacement for sizetube.com

This runbook clones the PeerTube `sizetube.com` database into Vidra while
...
- DNS first on a test name such as `test.sizetube.com`; only move
  `sizetube.com` after playback and login checks pass.
...
Review the JSON report. For the sizetube dump, expect a PeerTube schema version
...
```bash
curl -i https://test.sizetube.com/api/v1/videos/<vidra-video-id>/hls/master.m3u8
```
...
1. Start with `test.sizetube.com` pointed at the Droplet.
2. Verify API health, UI load, login, thumbnails, originals, and HLS playback.
3. Lower the `sizetube.com` A/AAAA record TTL.
4. Point `sizetube.com` at the Droplet IP.
