...
echo "[1/4] Enforcing gofmt on staged Go files..."
...
# 2) Run autonomous stop hooks for parity and completeness guardrails
echo "[2/4] Running autonomous stop hooks..."
if ./scripts/verify-autonomous-stop-hooks.sh; then
    echo "✓ Autonomous stop hooks passed"
else
    echo "✗ Autonomous stop hooks failed"
    FAILURES=$((FAILURES + 1))
fi

# 3) Lint (may auto-fix import groups if configured)
...
# 4) Run optional pre-commit hooks (e.g., yamllint) if available
if command -v pre-commit >/dev/null 2>&1; then
    if [ "$FAILURES" -gt 0 ]; then
        echo "[4/4] Skipping pre-commit hooks (previous checks failed)"
    else
        echo "[4/4] Running pre-commit hooks on staged files..."
    ...
    echo "[4/4] Skipping pre-commit hooks (not installed)"
fi
