...
test-integration-ci: ## Run repository + httpapi Integration tests (CI services env)
	@echo "Running integration tests (excluding load tests via build tag)..."
	@$(GO_ENV) go test -v -parallel=8 ./...

test-integration-ci-race: ## Run integration tests in CI with race detection (requires CGO_ENABLED=1 and gcc)
	@echo "Running integration tests with race detection (excluding load tests)..."
	@CGO_ENABLED=1 $(GO_ENV) go test -v -race -parallel=8 ./...
