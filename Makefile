...
test-ipfs-integration: ## Run real-Kubo tests; public proof additionally needs IPFS_TEST_PUBLIC_GATEWAY_URL + IPFS_TEST_VIDEO_PATH (self-skips unless required)
	go test -count=1 -v -tags=ipfs_integration -race ./internal/ipfs/... ./internal/ipfsmirror/...
...
test-ipfs-integration-local: ## Local-kubo IPFS proofs only (required CI lane); needs IPFS_TEST_API_URL + IPFS_TEST_GATEWAY_URL
	go test -count=1 -v -tags=ipfs_integration -race -run 'TestIntegrationAddPinCat|TestIntegrationAddDirectoryHLSTree|TestIntegrationMirrorPinsPromotedGeneration|TestIntegrationListPinsSeesWhatTheNodeHolds|TestIntegrationRepoGCRemovesUnpinnedBlocksOnly|TestIntegrationGatewayProbeFetchesAPinnedCID|TestIntegrationGatewayProbeReportsAnUnreachableGateway' ./internal/ipfs/... ./internal/ipfsmirror/...
