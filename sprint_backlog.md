...
**Tasks:**
- [x] Modify `verifyInfra` in `internal/testutil/database.go` (Verified: implemented).
- [x] Replace `connectWithRetry` calls with `net.DialTimeout("tcp", host:port, 100ms)` (Verified: implemented).
- [x] Apply this fast check for both Postgres and Redis (Verified: implemented).
- [x] Ensure `SetupTestDB` calls `t.Skip` immediately if the TCP check fails (Verified: behavior confirmed).
...
**Tasks:**
- [x] Create `scripts/rotate-credentials.sh` to generate strong random secrets (Verified: exists).
- [x] Create `scripts/setup-production-env.sh` (or update instructions) (Verified: exists).
...
- [x] Document the exact `git filter-branch` or `bfg` commands required.
...
**Tasks:**
- [x] Start test infra (`docker compose up -d postgres redis`).
- [x] Run `go test -v ./internal/repository/...`.
- [x] Fix any SQL syntax errors or schema mismatches found.

**Acceptance Criteria:**
- All repository tests pass with a running DB (or skip correctly without it).
...
- [x] Add Docker Login prerequisite to `README.md`.
