...
// Service handles business logic for playlists
type Service struct {
...
func NewService(playlistRepo port.PlaylistRepository, videoRepo port.VideoRepository) *Service {
	return &Service{
...
func (s *Service) CreatePlaylist(ctx context.Context, userID uuid.UUID, req *domain.CreatePlaylistRequest) (*domain.Playlist, error) {
...
func (s *Service) GetPlaylist(ctx context.Context, playlistID uuid.UUID, userID *uuid.UUID) (*domain.Playlist, error) {
...
func (s *Service) UpdatePlaylist(ctx context.Context, userID, playlistID uuid.UUID, req domain.UpdatePlaylistRequest) error {
	// Check ownership
...
func (s *Service) ListPlaylists(ctx context.Context, opts domain.PlaylistListOptions) (*domain.PlaylistListResponse, error) {
	// Set default limits
...
func (s *Service) AddVideoToPlaylist(ctx context.Context, userID, playlistID, videoID uuid.UUID, position *int) error {
	// Check ownership
	isOwner, err := s.playlistRepo.IsOwner(ctx, playlistID, userID)
	if err != nil {
		return fmt.Errorf("failed to check ownership: %w", err)
...
func (s *Service) RemoveVideoFromPlaylist(ctx context.Context, userID, playlistID, itemID uuid.UUID) error {
	// Check ownership
	isOwner, err := s.playlistRepo.IsOwner(ctx, playlistID, userID)
	if err != nil {
		return fmt.Errorf("failed to check ownership: %w", err)
...
func (s *Service) GetPlaylistItems(ctx context.Context, playlistID uuid.UUID, userID *uuid.UUID, limit, offset int) ([]*domain.PlaylistItem, error) {
	// Check privacy permissions
	playlist, err := s.playlistRepo.GetByID(ctx, playlistID)
	if err != nil {
		return nil, err
...
func (s *Service) ReorderPlaylistItem(ctx context.Context, userID, playlistID, itemID uuid.UUID, newPosition int) error {
	// Check ownership
	isOwner, err := s.playlistRepo.IsOwner(ctx, playlistID, userID)
	if err != nil {
		return fmt.Errorf("failed to check ownership: %w", err)
...
func (s *Service) GetOrCreateWatchLater(ctx context.Context, userID uuid.UUID) (*domain.Playlist, error) {
...
func (s *Service) AddToWatchLater(ctx context.Context, userID, videoID uuid.UUID) error {
