...
// SocialService handles ATProto social interactions
...
func (s *SocialService) Follow(ctx context.Context, followerDID, targetHandle string) error {
...
func (s *SocialService) Unfollow(ctx context.Context, followerDID, targetHandle string) error {
	targetActor, err := s.resolveActor(ctx, targetHandle)
	if err != nil {
		return fmt.Errorf("resolve target actor: %w", err)
	}
...
func (s *SocialService) Like(ctx context.Context, actorDID, subjectURI, subjectCID string) error {
...
func (s *SocialService) Unlike(ctx context.Context, actorDID, subjectURI string) error {
	// Get like record
...
func (s *SocialService) DeleteComment(ctx context.Context, uri string) error {
	// Delete record from ATProto
...
func (s *SocialService) ApplyModerationLabel(
	ctx context.Context,
	actorDID string,
	labelType string,
	reason string,
	...
	if s.cfg.EnableATProtoLabeler {
		if err := s.createLabel(ctx, label); err != nil {
			return fmt.Errorf("create ATProto label: %w", err)
		}
	}
...
func (s *SocialService) RemoveModerationLabel(ctx context.Context, labelID string) error {
	return s.socialRepo.RemoveModerationLabel(ctx, labelID)
}

// GetSocialStats retrieves social statistics for an actor
func (s *SocialService) GetSocialStats(ctx context.Context, handle string) (*domain.SocialStats, error) {
...
// ResolveActor resolves an actor by handle or DID
func (s *SocialService) ResolveActor(ctx context.Context, handle string) (*domain.ATProtoActor, error) {
	return s.resolveActor(ctx, handle)
}

// GetFollowers retrieves followers of an actor
func (s *SocialService) GetFollowers(ctx context.Context, handle string, limit, offset int) ([]domain.Follow, error) {
	actor, err := s.resolveActor(ctx, handle)
	if err != nil {
		return nil, err
	}
...
func (s *SocialService) GetFollowing(ctx context.Context, handle string, limit, offset int) ([]domain.Follow, error) {
	actor, err := s.resolveActor(ctx, handle)
	if err != nil {
		return nil, err
	}
...
func (s *SocialService) GetLikes(ctx context.Context, subjectURI string, limit, offset int) ([]domain.Like, error) {
	return s.socialRepo.GetLikes(ctx, subjectURI, limit, offset)
}

// GetComments retrieves comments for a subject
func (s *SocialService) GetComments(ctx context.Context, rootURI string, limit, offset int) ([]domain.SocialComment, error) {
	return s.socialRepo.GetComments(ctx, rootURI, limit, offset)
}

// GetCommentThread retrieves a comment thread
func (s *SocialService) GetCommentThread(ctx context.Context, parentURI string, limit, offset int) ([]domain.SocialComment, error) {
...
func (s *SocialService) GetModerationLabels(ctx context.Context, did string) ([]domain.ModerationLabel, error) {
	return s.socialRepo.GetModerationLabels(ctx, did)
}

// IngestActorFeed ingests posts from an actor's feed
func (s *SocialService) IngestActorFeed(ctx context.Context, handle string, limit int) error {
	actor, err := s.resolveActor(ctx, handle)
	if err != nil {
	...
	// Fetch feed from ATProto
	feed, err := s.getActorFeed(ctx, actor.DID, limit)
	if err != nil {
		return fmt.Errorf("get actor feed: %w", err)
	}

	// Process each post
	posts := feed["feed"].([]interface{})
	for _, p := range posts {
		...
		// Process likes
		if likes, ok := post["likeCount"].(float64); ok && likes > 0 {
			s.ingestPostLikes(ctx, post["uri"].(string), post["cid"].(string))
		}

		// Process comments/replies
		if reply, ok := post["record"].(map[string]interface{})["reply"]; ok && reply != nil {
			s.processReply(ctx, post)
		}
	}
...
func (s *SocialService) resolveActor(ctx context.Context, handle string) (*domain.ATProtoActor, error) {
	// Check cache first
	actor, err := s.socialRepo.GetActorByHandle(ctx, handle)
	if err == nil {
		return actor, nil
	}

	// Resolve from ATProto
	resolvedActor, err := s.resolveActorFromNetwork(ctx, handle)
	if err != nil {
		return nil, err
	}

	// Store in database
	if err := s.socialRepo.UpsertActor(ctx, resolvedActor); err != nil {
		return nil, err
	}
...
func (s *SocialService) resolveActorFromNetwork(ctx context.Context, handle string) (*domain.ATProtoActor, error) {
...
func (s *SocialService) getProfile(ctx context.Context, did string) (*domain.ATProtoActor, error) {
	pds := strings.TrimRight(s.cfg.ATProtoPDSURL, "/")
...
func (s *SocialService) createRecord(ctx context.Context, repoDID, collection string, record interface{}) (string, string, error) {
	pds := strings.TrimRight(s.cfg.ATProtoPDSURL, "/")
...
func (s *SocialService) deleteRecord(ctx context.Context, uri string) error {
	pds := strings.TrimRight(s.cfg.ATProtoPDSURL, "/")
...
func (s *SocialService) getActorFeed(ctx context.Context, did string, limit int) (map[string]interface{}, error) {
	pds := strings.TrimRight(s.cfg.ATProtoPDSURL, "/")
...
func (s *SocialService) getAccessToken(ctx context.Context) (string, error) {
	// This would integrate with the atproto service's session management
	// For now, returning a placeholder
	return s.cfg.ATProtoAppPassword, nil
}

func (s *SocialService) createLabel(ctx context.Context, label *domain.ModerationLabel) error {
	// This would integrate with ATProto labeler service
	// Implementation depends on labeler service setup
	return nil
}

func (s *SocialService) shouldBlock(post map[string]interface{}, blockedLabels []string) bool {
...
func (s *SocialService) ingestPostLikes(ctx context.Context, uri, cid string) {
	// This would fetch and ingest likes for a post
	// Implementation depends on requirements
}

func (s *SocialService) processReply(ctx context.Context, post map[string]interface{}) {
