...
// List retrieves a paginated list of videos. It delegates to the
// repository layer and returns videos ordered by creation time.
func (uc *VideoUsecase) List(ctx context.Context, limit, offset int) ([]*model.Video, error) {
	if limit <= 0 {
		limit = 20
	}
	return uc.Videos.List(ctx, limit, offset)
}
