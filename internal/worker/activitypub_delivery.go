...
func (w *ActivityPubDeliveryWorker) Start(ctx context.Context) error {
	slog.Info(fmt.Sprintf("Starting ActivityPub delivery worker with %d workers", w.cfg.ActivityPubDeliveryWorkers))
...
func (w *ActivityPubDeliveryWorker) Stop() error {
	slog.Info("Stopping ActivityPub delivery worker")
	close(w.stopCh)
	return nil
}
...
func (w *ActivityPubDeliveryWorker) deliveryLoop(ctx context.Context, workerID int) {
	...
	slog.Info(fmt.Sprintf("ActivityPub delivery worker %d started", workerID))

	for {
		select {
		case <-ctx.Done():
			slog.Info(fmt.Sprintf("ActivityPub delivery worker %d stopped (context cancelled)", workerID))
			return
		case <-w.stopCh:
			slog.Info(fmt.Sprintf("ActivityPub delivery worker %d stopped (stop signal)", workerID))
			return
		case <-ticker.C:
...
func (w *ActivityPubDeliveryWorker) processDeliveries(ctx context.Context, workerID int) {
	...
	if err != nil {
		slog.Info(fmt.Sprintf("Worker %d: failed to get pending deliveries: %v", workerID, err))
		return
	}
	...
	slog.Info(fmt.Sprintf("Worker %d: processing %d deliveries", workerID, len(deliveries)))
	...
	for _, delivery := range deliveries {
		...
		if err != nil {
			slog.Info(fmt.Sprintf("Worker %d: failed to update delivery status: %v", workerID, err))
			continue
		}
		...
		if err != nil {
			slog.Info(fmt.Sprintf("Worker %d: delivery %s failed (attempt %d/%d): %v",
			...
			if delivery.Attempts >= delivery.MaxAttempts {
				...
				slog.Info(fmt.Sprintf("Worker %d: delivery %s permanently failed after %d attempts", workerID, delivery.ID, delivery.Attempts))
			} else {
				...
				slog.Info(fmt.Sprintf("Worker %d: delivery %s scheduled for retry at %s", workerID, delivery.ID, nextAttempt))
			}
		} else {
			// Mark as completed
			slog.Info(fmt.Sprintf("Worker %d: delivery %s completed successfully", workerID, delivery.ID))
			_ = w.apRepo.UpdateDeliveryStatus(ctx, port.DeliveryStatusParams{
