...
func writeMinimalMP4(path string, totalSize int) error {
    ...
    if _, err := f.Write(payload); err != nil { return err }
    return nil
}
...
func TestUploadWithActualVideoFile(t *testing.T) {
    ...
    var env Response
    if err := json.Unmarshal(w.Body.Bytes(), &env); err != nil { t.Fatal(err) }
    if !env.Success { t.Fatalf("unexpected error: %+v", env.Error) }

    var initResp domain.InitiateUploadResponse
    dataBytes, _ := json.Marshal(env.Data)
    if err := json.Unmarshal(dataBytes, &initResp); err != nil { t.Fatal(err) }
    ...
    for i := 0; i < totalChunks; i++ {
        ...
        if !env2.Success { t.Fatalf("chunk error: %+v", env2.Error) }
    }
    ...
    if err != nil { t.Fatalf("read assembled: %v", err) }
...
func TestUploadLargeVideo_VariousChunkSizes(t *testing.T) {
    ...
    fileBytes, err := os.ReadFile(samplePath)
    if err != nil { t.Fatalf("read sample: %v", err) }
    ...
    for _, chunkSize := range chunkSizes {
        ...
        var env Response
        if err := json.Unmarshal(w.Body.Bytes(), &env); err != nil { t.Fatal(err) }
        if !env.Success { t.Fatalf("init env error: %+v", env.Error) }
        var initResp domain.InitiateUploadResponse
        dataBytes, _ := json.Marshal(env.Data)
        if err := json.Unmarshal(dataBytes, &initResp); err != nil { t.Fatal(err) }
...
func TestResumeUploadWithActualVideoFile(t *testing.T) {
    ...
    w := httptest.NewRecorder()
    InitiateUploadHandler(uploadService, videoRepo)(w, httpReq)
    if w.Code != http.StatusCreated { t.Fatalf("initiate failed: %d %s", w.Code, w.Body.String()) }
    var env Response
    if err := json.Unmarshal(w.Body.Bytes(), &env); err != nil { t.Fatal(err) }
    if !env.Success { t.Fatalf("unexpected error: %+v", env.Error) }
    var initResp domain.InitiateUploadResponse
    ...
    for i := 0; i < totalChunks && i < 5; i++ {
        ...
        w = httptest.NewRecorder()
        UploadChunkHandler(uploadService, createTestConfig())(w, httpReq)
        if w.Code != http.StatusOK { t.Fatalf("upload i=%d failed: %d %s", i, w.Code, w.Body.String()) }
        uploaded[i] = true
    }
    ...
    if w.Code != http.StatusOK { t.Fatalf("resume failed: %d %s", w.Code, w.Body.String()) }
    var env2 Response
    if err := json.Unmarshal(w.Body.Bytes(), &env2); err != nil { t.Fatal(err) }
    if !env2.Success { t.Fatalf("resume error: %+v", env2.Error) }
    var resumeMap map[string]interface{}
    dataBytes2, _ := json.Marshal(env2.Data)
    if err := json.Unmarshal(dataBytes2, &resumeMap); err != nil { t.Fatal(err) }
    // Basic checks
    if resumeMap["session_id"].(string) != initResp.SessionID { t.Fatal("session mismatch") }
    ...
    w = httptest.NewRecorder()
    CompleteUploadHandler(uploadService, encodingRepo)(w, httpReq)
    if w.Code != http.StatusOK { t.Fatalf("complete failed: %d %s", w.Code, w.Body.String()) }
    ...
    out, err := os.ReadFile(assembledPath)
    if err != nil { t.Fatalf("read assembled: %v", err) }
    if !bytes.Equal(fileBytes, out) { t.Fatalf("assembled mismatch: got %d want %d", len(out), len(fileBytes)) }
}
...
func TestUploadResumeAfterChecksumMismatch_WithVideoFile(t *testing.T) {
    ...
    if err := writeMinimalMP4(samplePath, 128*1024); err != nil { t.Fatalf("create sample: %v", err) }
    fileBytes, err := os.ReadFile(samplePath)
    if err != nil { t.Fatalf("read sample: %v", err) }
    ...
    if badIndex < totalChunks {
        ...
        w = httptest.NewRecorder()
        UploadChunkHandler(uploadService, cfg)(w, httpReq)
        if w.Code != http.StatusBadRequest { t.Fatalf("expected 400 on mismatch, got %d", w.Code) }
        var envErr Response
        _ = json.Unmarshal(w.Body.Bytes(), &envErr)
        if envErr.Success || envErr.Error == nil || envErr.Error.Code == "" { t.Fatalf("expected error envelope on mismatch") }
    }
    ...
    if len(ulist) < 2 { t.Fatalf("expected at least 2 uploaded chunks, got %d", len(ulist)) }
...
func TestStrictModeRequiresChecksum(t *testing.T) {
    ...
    if err := writeMinimalMP4(samplePath, 64*1024); err != nil { t.Fatalf("create sample: %v", err) }
    ...
    _ = json.Unmarshal(w.Body.Bytes(), &env)
    var initResp domain.InitiateUploadResponse
    db, _ := json.Marshal(env.Data); _ = json.Unmarshal(db, &initResp)
    ...
    if w.Code != http.StatusBadRequest { t.Fatalf("expected 400 in strict mode missing checksum, got %d", w.Code) }
