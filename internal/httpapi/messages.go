...
// GetMessageThreadHandler handles retrieving messages in a specific thread
func GetMessageThreadHandler(messageService *usecase.MessageService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		userID, _ := r.Context().Value(middleware.UserIDKey).(string)
		if userID == "" {
			WriteError(w, http.StatusUnauthorized, domain.NewDomainError("UNAUTHORIZED", "Missing or invalid authentication"))
		...
		if threadID == "" {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("MISSING_PARAMETER", "threadId parameter is required"))
			return
		}
		...
		req := &domain.GetMessageThreadRequest{
			ThreadID: threadID,
			Limit:    limit,
			Offset:   offset,
		}
		...
		messages, err := messageService.GetMessageThread(r.Context(), userID, req)
		if err != nil {
			status := MapDomainErrorToHTTP(err)
			WriteError(w, status, domain.NewDomainError("GET_THREAD_FAILED", err.Error()))
			return
		}
...
// UpdateMessageHandler handles editing a message (within 5 minutes)
func UpdateMessageHandler(messageService *usecase.MessageService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		userID, _ := r.Context().Value(middleware.UserIDKey).(string)
		if userID == "" {
			WriteError(w, http.StatusUnauthorized, domain.NewDomainError("UNAUTHORIZED", "Missing or invalid authentication"))
		...
		message, err := messageService.UpdateMessage(r.Context(), userID, &req)
		if err != nil {
			status := MapDomainErrorToHTTP(err)
			WriteError(w, status, domain.NewDomainError("UPDATE_MESSAGE_FAILED", err.Error()))
			return
		}

		WriteJSON(w, http.StatusOK, domain.MessageResponse{Message: *message})
	}
}
