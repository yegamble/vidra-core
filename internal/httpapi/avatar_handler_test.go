...
func TestUploadAvatar_ValidPNG_WithMockIPFS(t *testing.T) {
    ...
    // Override test hooks to bypass network
    testIPFSAdd = func(localPath string) (string, error) { return "CID123", nil }
    ...
    if got.AvatarFileID == nil || *got.AvatarFileID == "" {
        t.Fatalf("expected avatar file id set, got %+v", got)
    }
