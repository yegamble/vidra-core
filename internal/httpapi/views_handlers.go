...
func (h *ViewsHandler) TrackView(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("TRACKING_FAILED", "Failed to track view"))
		return
	}
...
func (h *ViewsHandler) GetVideoAnalytics(w http.ResponseWriter, r *http.Request) {
	...
	if startDateStr := r.URL.Query().Get("start_date"); startDateStr != "" {
		...
		if err != nil {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_DATE", "Invalid start_date format. Use YYYY-MM-DD"))
			return
		}
	...
	if endDateStr := r.URL.Query().Get("end_date"); endDateStr != "" {
		...
		if err != nil {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_DATE", "Invalid end_date format. Use YYYY-MM-DD"))
			return
		}
	...
	if isAnonymousStr := r.URL.Query().Get("is_anonymous"); isAnonymousStr != "" {
		...
		if err != nil {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_BOOLEAN", "Invalid is_anonymous value"))
			return
		}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("ANALYTICS_FAILED", "Failed to get video analytics"))
		return
	}
...
func (h *ViewsHandler) GetDailyStats(w http.ResponseWriter, r *http.Request) {
	...
	if daysStr := r.URL.Query().Get("days"); daysStr != "" {
		...
		if err != nil || parsedDays <= 0 || parsedDays > 365 {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_DAYS", "Days must be between 1 and 365"))
			return
		}
		days = parsedDays
	}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("STATS_FAILED", "Failed to get daily stats"))
		return
	}
...
func (h *ViewsHandler) GetUserEngagement(w http.ResponseWriter, r *http.Request) {
	...
	if authUserID.(string) != userID {
		...
		WriteError(w, http.StatusForbidden, domain.NewDomainError("FORBIDDEN", "Cannot access other user's engagement data"))
		return
	}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("ENGAGEMENT_FAILED", "Failed to get user engagement"))
		return
	}
...
func (h *ViewsHandler) GetTopVideos(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("TOP_VIDEOS_FAILED", "Failed to get top videos"))
		return
	}
...
func (h *ViewsHandler) GetViewHistory(w http.ResponseWriter, r *http.Request) {
	...
	if offsetStr := r.URL.Query().Get("offset"); offsetStr != "" {
		...
		if err != nil || offset < 0 {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_OFFSET", "Offset must be a non-negative integer"))
			return
		}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("HISTORY_FAILED", "Failed to get view history"))
		return
	}
...
func (h *ViewsHandler) GenerateFingerprint(w http.ResponseWriter, r *http.Request) {
	...
	if request.IP == "" || request.UserAgent == "" {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("MISSING_FIELDS", "IP and user_agent are required"))
		return
	}
...
func (h *ViewsHandler) AdminAggregateStats(w http.ResponseWriter, r *http.Request) {
	...
	if request.Date != nil && *request.Date != "" {
		...
		if err != nil {
			WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_DATE", "Invalid date format. Use YYYY-MM-DD"))
			return
		}
		aggregateDate = &date
	}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("AGGREGATION_FAILED", "Failed to aggregate stats"))
		return
	}
...
func (h *ViewsHandler) AdminCleanupOldData(w http.ResponseWriter, r *http.Request) {
	...
	if request.DaysToKeep <= 0 || request.DaysToKeep > 3650 { // Max 10 years
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_DAYS", "Days to keep must be between 1 and 3650"))
		return
	}

	err := h.viewsService.CleanupOldData(r.Context(), request.DaysToKeep)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, domain.NewDomainError("CLEANUP_FAILED", "Failed to cleanup old data"))
		return
	}
