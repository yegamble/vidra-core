...
func (h *HLSHandlers) GetMasterPlaylist(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_STREAM_ID", "Invalid stream ID"))
		return
	}
	...
	if err != nil {
		WriteError(w, http.StatusNotFound, domain.NewDomainError("STREAM_NOT_FOUND", "Stream not found"))
		return
	}
	...
	if !stream.IsLive() {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("STREAM_NOT_LIVE", "Stream is not currently live"))
		return
	}
	...
	if _, err := os.Stat(playlistPath); os.IsNotExist(err) {
		WriteError(w, http.StatusNotFound, domain.NewDomainError("PLAYLIST_NOT_FOUND", "HLS playlist not yet available"))
		return
	}
...
func (h *HLSHandlers) GetVariantPlaylist(w http.ResponseWriter, r *http.Request) {
	...
	if variant == "" {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_VARIANT", "Variant parameter required"))
		return
	}

	// Validate variant name (prevent path traversal)
	if !isValidVariantName(variant) {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_VARIANT", "Invalid variant name"))
		return
	}
	...
	if _, err := os.Stat(playlistPath); os.IsNotExist(err) {
		WriteError(w, http.StatusNotFound, domain.NewDomainError("PLAYLIST_NOT_FOUND", "Variant playlist not found"))
		return
	}
...
func (h *HLSHandlers) GetSegment(w http.ResponseWriter, r *http.Request) {
	...
	if segment == "" {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_SEGMENT", "Segment parameter required"))
		return
	}

	// Validate names (prevent path traversal)
	if !isValidVariantName(variant) || !isValidSegmentName(segment) {
		WriteError(w, http.StatusBadRequest, domain.NewDomainError("INVALID_PATH", "Invalid path parameters"))
		return
	}
	...
	if _, err := os.Stat(segmentPath); os.IsNotExist(err) {
		WriteError(w, http.StatusNotFound, domain.NewDomainError("SEGMENT_NOT_FOUND", "Segment not found"))
		return
	}
