...
// SetPGPPublicKey sets or updates the user's PGP public key
func (r *userRepository) SetPGPPublicKey(ctx context.Context, userID string, pgpPublicKey string) error {
	query := `UPDATE users SET pgp_public_key = $2, updated_at = NOW() WHERE id = $1`
	res, err := r.db.ExecContext(ctx, query, userID, pgpPublicKey)
	if err != nil {
...
func (r *userRepository) SetPGPPublicKeyWithFingerprint(ctx context.Context, userID string, pgpPublicKey string, fingerprint string) error {
	query := `UPDATE users SET pgp_public_key = $2, pgp_fingerprint = $3, updated_at = NOW() WHERE id = $1`
	res, err := r.db.ExecContext(ctx, query, userID, pgpPublicKey, fingerprint)
	if err != nil {
		return fmt.Errorf("failed to set pgp key and fingerprint: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return domain.ErrUserNotFound
	}
...
// RemovePGPPublicKey removes the user's PGP public key
func (r *userRepository) RemovePGPPublicKey(ctx context.Context, userID string) error {
	query := `UPDATE users SET pgp_public_key = NULL, updated_at = NOW() WHERE id = $1`
...
// GetPGPPublicKey fetches the user's PGP public key as a nullable string pointer
func (r *userRepository) GetPGPPublicKey(ctx context.Context, userID string) (*string, error) {
	var s sql.NullString
	if err := r.db.GetContext(ctx, &s, `SELECT pgp_public_key FROM users WHERE id = $1`, userID); err != nil {
		if err == sql.ErrNoRows {
			return nil, domain.ErrUserNotFound
		}
...
func (r *userRepository) GetPGPFingerprint(ctx context.Context, userID string) (*string, error) {
	var s sql.NullString
	if err := r.db.GetContext(ctx, &s, `SELECT pgp_fingerprint FROM users WHERE id = $1`, userID); err != nil {
		if err == sql.ErrNoRows {
			return nil, domain.ErrUserNotFound
		}
