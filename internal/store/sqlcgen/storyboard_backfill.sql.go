...
// versions:
//   sqlc v1.31.1
// source: storyboard_backfill.sql
...
const clearStoryboardAttempt = `-- name: ClearStoryboardAttempt :exec
...
func (q *Queries) ClearStoryboardAttempt(ctx context.Context, videoID uuid.UUID) error {
	_, err := q.db.Exec(ctx, clearStoryboardAttempt, videoID)
	return err
}

const countAbandonedStoryboards = `-- name: CountAbandonedStoryboards :one
...
func (q *Queries) CountAbandonedStoryboards(ctx context.Context) (int64, error) {
	row := q.db.QueryRow(ctx, countAbandonedStoryboards)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const giveUpOnStoryboard = `-- name: GiveUpOnStoryboard :exec
INSERT INTO video_storyboard_attempts (video_id, attempts, next_attempt_at, last_error, given_up)
VALUES ($1, 1, now(), $2, true)
...
type GiveUpOnStoryboardParams struct {
...
func (q *Queries) GiveUpOnStoryboard(ctx context.Context, arg GiveUpOnStoryboardParams) error {
	_, err := q.db.Exec(ctx, giveUpOnStoryboard, arg.VideoID, arg.LastError)
	return err
}

const listVideosNeedingStoryboard = `-- name: ListVideosNeedingStoryboard :many
...
type ListVideosNeedingStoryboardRow struct {
...
func (q *Queries) ListVideosNeedingStoryboard(ctx context.Context, limit int32) ([]ListVideosNeedingStoryboardRow, error) {
	rows, err := q.db.Query(ctx, listVideosNeedingStoryboard, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListVideosNeedingStoryboardRow
	for rows.Next() {
		var i ListVideosNeedingStoryboardRow
		if err := rows.Scan(
			&i.ID,
			&i.StorageKey,
			&i.DurationSeconds,
...
const recordStoryboardAttemptFailure = `-- name: RecordStoryboardAttemptFailure :one
INSERT INTO video_storyboard_attempts (video_id, attempts, next_attempt_at, last_error, given_up)
VALUES (
    $1,
    1,
...
SET attempts        = video_storyboard_attempts.attempts + 1,
    ...
    last_error      = EXCLUDED.last_error,
    given_up        = video_storyboard_attempts.attempts + 1 >= $4::int,
    updated_at      = now()
RETURNING attempts, given_up
`

type RecordStoryboardAttemptFailureParams struct {
	VideoID       uuid.UUID `json:"video_id"`
...
type RecordStoryboardAttemptFailureRow struct {
	Attempts int32 `json:"attempts"`
...
func (q *Queries) RecordStoryboardAttemptFailure(ctx context.Context, arg RecordStoryboardAttemptFailureParams) (RecordStoryboardAttemptFailureRow, error) {
	row := q.db.QueryRow(ctx, recordStoryboardAttemptFailure,
		arg.VideoID,
		arg.NextAttemptAt,
		arg.LastError,
		arg.MaxAttempts,
	)
	var i RecordStoryboardAttemptFailureRow
	err := row.Scan(&i.Attempts, &i.GivenUp)
	return i, err
}
