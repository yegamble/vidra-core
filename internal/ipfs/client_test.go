...
func TestClient_AddAndPin_PinFailurePropagated(t *testing.T) {
	const testCID = "bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi"
	// Mock IPFS server: add succeeds, pin fails
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/v0/add":
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{"Name":"test.txt","Hash":"` + testCID + `","Size":"11"}`))
		case "/api/v0/pin/add":
			w.WriteHeader(http.StatusInternalServerError)
			_, _ = w.Write([]byte(`pin service unavailable`))
		default:
			w.WriteHeader(http.StatusNotFound)
		}
	}))
	...
	tmpDir := t.TempDir()
	testFile := filepath.Join(tmpDir, "test.txt")
	if err := os.WriteFile(testFile, []byte("hello ipfs"), 0644); err != nil {
		t.Fatal(err)
	}

	cid, err := client.AddAndPin(context.Background(), testFile)
	// Error should be returned (pin failed)
	if err == nil {
		t.Fatal("expected error when pin fails")
	}
	if !strings.Contains(err.Error(), "pin after add") {
		t.Fatalf("unexpected error: %v", err)
	}
	// CID should still be returned so caller can use it
	if cid != testCID {
		t.Fatalf("expected CID to be returned even on pin failure, got %q", cid)
	}
}

func TestClient_AddAndPin_Success(t *testing.T) {
	const testCID = "bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/v0/add":
	...
	cid, err := client.AddAndPin(context.Background(), testFile)
	if err != nil {
		t.Fatalf("AddAndPin() unexpected error: %v", err)
	}
	if cid != testCID {
		t.Fatalf("AddAndPin() CID = %q, want %q", cid, testCID)
	}
}

func TestClient_AddDirectoryAndPin_PinFailurePropagated(t *testing.T) {
	const testCID = "bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/v0/add":
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{"Name":"","Hash":"` + testCID + `","Size":"300"}`))
		case "/api/v0/pin/add":
			w.WriteHeader(http.StatusInternalServerError)
			_, _ = w.Write([]byte(`pin service unavailable`))
		default:
	...
	if err := os.WriteFile(filepath.Join(testDir, "file.txt"), []byte("data"), 0644); err != nil {
		t.Fatal(err)
	}

	cid, err := client.AddDirectoryAndPin(context.Background(), testDir)
	if err == nil {
		t.Fatal("expected error when pin fails")
	}
	if !strings.Contains(err.Error(), "pin after directory add") {
		t.Fatalf("unexpected error: %v", err)
	}
	if cid != testCID {
		t.Fatalf("expected CID to be returned even on pin failure, got %q", cid)
