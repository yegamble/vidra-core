...
- [~] Add watched words lists and matches tables. (Words list done: migration 0023 `watched_words` (id, `word`, `created_by` FK → users `ON DELETE SET NULL`, `created_at`; unique index on `lower(word)`). The **matches** table (which content matched which term) lands with the matching/flagging effect — a later slice.)
...
- [ ] Implement watched words tagging for videos/comments. (The **matching/flagging effect** — auto-holding or hiding a comment (then video) that contains a watched term, recorded in a matches table — is the next slice; this loop landed the list management contract.)
