...
minted one) and the per-request log line carries it as `correlation_id`. Every
response also carries security headers (`X-Content-Type-Options: nosniff`,
`X-Frame-Options: DENY`, `Referrer-Policy`, `Cross-Origin-Opener-Policy`,
`X-Permitted-Cross-Domain-Policies`; plus HSTS in production). `make
