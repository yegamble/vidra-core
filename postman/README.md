...
| # | Collection | Requests | Purpose | In CI |
|---|-----------|----------|---------|-------|
| 36 | **athena-e2e-auth-flow** | 6 | Register -> verify -> login -> 2FA -> logout -> re-login | Yes |
| 37 | **athena-e2e-video-lifecycle** | 8 | Login -> channel -> video -> caption -> rate -> comment | Yes |
| 38 | **athena-e2e-payment-flow** | 7 | Register users -> wallets -> payment intent -> history | Yes |

### Standalone Collections (not in CI runner)

| Collection | Purpose |
|-----------|---------|
| **athena-registration-edge-cases** | Username length limits, special chars, security tests |
| **athena-edge-cases-security** | Additional security edge case testing |
| **athena-virus-scanner-tests** | Virus scanner integration tests |
| **athena-frontend-api-gaps** | Frontend API gap coverage | Yes |
...
The runner executes all 38 collections in a stateful sequence:
...
**Total**: 38+ collections in CI, covering all major API surfaces.
